...
<html>
	...
	<body>
		...
		<h2 style = "color: #000066; text-align:center; font-weight:bolder"> 列車結果 </h2>
		...
		<div> 
			...
			<img src =" knhl.png" class = "picture_10"> 
			<img src =" 火車卡通圖-1200x628 (1).jpg" class = "picture_8">
		</div>
		
		<div> 
			<font size = "1" style="text-align:center"> 
				<a href="個人訂票.html" style="text-decoration:none; color:#000066; text-align:center;" class = "word">
					<h1>立即訂票</h1> 
				</a>
			</font>
		</div>
		...
		<div class = "lala"> 
			<font size = "1"> 
				<a href="查詢列車.html" style="text-decoration:none; color:#000066;">
					<h1>返回查詢</h1> 
				</a>
			</font>
		</div>
		...
		<table width="800" style="border:3px #000066 dashed" border="1" align="center"> 
			...
			<?php
				...
				/*	print ($query_1);
					...
					/*while($row_start_station = mysqli_fetch_row( $result_start_station )) 
					{
						while($row_end_station = mysqli_fetch_row( $result_end_station ))
						{
							for( $i = 0; $i < count($row_start_station) ; $i++ )
							{	
								print( "<td>$row_start_station[$i]</td>" );
								print( "<td>$row_end_station[$i]</td>" );
							}
						}
					}
					while( $row_1 = mysqli_fetch_row( $result_1 ) )
					{
						for( $i = 4; $i < count($row_1); $i++ )
						{	
							print("<td>$row_1[$i]</td>" );
				...
				if ( $num == null)
				{
					print ("<script> window.alert('目前暫無列車') 
								location.href='http://localhost/%E8%BB%9F%E5%8A%9F/%E6%9F%A5%E8%A9%A2%E5%88%97%E8%BB%8A.html'
							</script>");
				}
			?>
		</table>
